...
export function RowEditorPanel({ open, mode, connectionId, schema, table, columns, pkColumns, row, stagedValues, onClose, onStageEdit, onStageInsert, onDeleteRow }: RowEditorPanelProps) {
  ...
  const isRequired = (col: RowEditorColumn) =>
    !col.isIdentity && !col.isNullable && (col.defaultValue === null || col.defaultValue === undefined || col.defaultValue === '');
  ...
  const handleInsert = () => {
    ...
    // Identity columns are auto-generated by the DB — don't send them.
    const insertable = columns.filter(col => !col.isIdentity);
    const filled = insertable.filter(col => (values[col.columnName] ?? '') !== '');
    let query: string;
    let params: unknown[];
