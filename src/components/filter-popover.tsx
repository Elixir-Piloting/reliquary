    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-80 p-2" align="start">
        <div className="flex items-center justify-between px-1 pb-2">
          ...
          <Button variant="ghost" size="sm" className="h-6 px-2 text-xs" onClick={addRow}><Plus className="h-3 w-3 mr-1" />Add filter</Button>
        </div>
        <div className="space-y-2">
          {rows.length === 0 && (
            <p className="px-1 py-4 text-center text-xs text-muted-foreground">No filters</p>
          )}
          {rows.map(row => {
            ...
            return (
              <div key={row.id} className="flex items-start gap-2 rounded-md border border-border p-2">
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  {!noValue && (
                    <Input value={row.value} onChange={e => updateRow(row.id, { value: e.target.value })} placeholder="Value" className="h-7 text-xs" />
                  )}
                </div>
