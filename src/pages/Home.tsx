...
import { Plus, Search, Loader2, Pencil, Copy, Trash2, EllipsisVertical, ShieldCheck, Database } from "lucide-react";
...
const PROVIDER_BADGES: Record<string, { label: string; dot: string; text: string; ring: string }> = {
  neon: {
    ...
    dot: "bg-[#00E599]",
    text: "text-[#00E599]",
    ring: "border-[#00E599]/30 bg-[#00E599]/10",
  },
  supabase: {
    label: "Supabase",
    dot: "bg-[#3ECF8E]",
        ...
        <div className="max-w-3xl mx-auto p-8">
          <div className="space-y-8 pt-4">
            <div className="flex items-end justify-between gap-6">
              ...
              <Button onClick={() => navigate("/add-connection")} className="gap-2 shrink-0">
            ...
            <div className="relative w-full">
              ...
              <Input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search connections..." className="pl-9 h-10" />
            </div>
            ...
            {isLoading ? (
              ...
              <div className="grid gap-3 no-ring">
                {filteredConnections.map(conn => (
                  <div key={conn.id} className="flex items-center justify-between p-4 rounded-xl border bg-card hover:bg-accent/50 hover:border-accent-foreground/20 transition-colors">
                    <button onClick={() => handleConnectionSelect(conn)} className="flex items-center gap-4 min-w-0 flex-1 text-left">
                      <ProviderLogo provider={conn.provider} className="h-12 w-12" />
                      <div className="min-w-0">
                        <p className="font-medium text-base truncate">{conn.name}</p>
